...
# quit function, is always called on by new screens, so it is possible
# to quit without errors
...
# quit button, initiates "quit" when mouse is clicked on a square with given
# coordinates (coordinates should be the same where "quit" button is
# displayed in graphics.py)
...
# three buttons. all launch "choose x or o screen". all close "choose
# difficulty screen" # game_ai difficulty in settings is changed depending
# on which difficulty is chosen # this will result in different ai being
# called for ai_turn in game_logic.py
def choose_diff_button(game_settings, screen, w_u, h_u, event):
    mouse = pygame.mouse.get_pos()
    if event.type == pygame.MOUSEBUTTONDOWN:
        if w_u * 3 <= mouse[0] <= w_u * 4.1 and h_u * 2.8 <= mouse[1] \
                <= h_u * 3.6:
        ...
        elif w_u * 3 <= mouse[0] <= w_u * 4.5 and h_u * 3.8 <= mouse[1] \
                <= h_u * 4.6:
            game_settings.diff_level = "normal"
            game_settings.diff_screen_on = False
            game_settings.xo_screen_on = True
            gsq.choose_x_o(game_settings, screen, w_u, h_u)
        elif w_u * 3 <= mouse[0] <= w_u * 4.1 and h_u * 4.8 <= mouse[1] \
                <= h_u * 5.6:
