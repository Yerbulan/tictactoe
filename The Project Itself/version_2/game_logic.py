...
# checks whether the game is over (through other function) and if not gives
# the turn to either AI
...
def check_turn(game_settings, screen, w_u, h_u):
    if game_settings.game_active:
        ...
        game_settings.available_squares = [[2, 4, 2, 4], [4, 6, 2, 4],
                                           [6, 8, 2, 4], [2, 4, 4, 6],
                                           [4, 6, 4, 6], [6, 8, 4, 6],
                                           [2, 4, 6, 8], [4, 6, 6, 8],
                                           [6, 8, 6, 8]]
...
def players_turn(game_settings, screen, w_u, h_u):
    while game_settings.players_turn:
        xo = "X" if game_settings.x_or_o_ == "x" else "O"
        for event in pygame.event.get():
            mouse = pygame.mouse.get_pos()
            gi.check_events(event)
            if event.type == pygame.MOUSEBUTTONDOWN:
                for square in game_settings.available_squares:
                    if w_u * square[0] <= mouse[0] <= w_u * square[1] and \
                            h_u * square[2] <= mouse[1] <= h_u * square[3]:
                        game_settings.available_squares.remove(square)
                        game_settings.player_squares.add(str(square))
                        gs.create_onscreen_text(
                            game_settings, screen, w_u * (square[0] + 0.6),
                            h_u * (square[2] + 0.1), xo, 1)
                        pygame.display.update()
                        game_settings.players_turn = False
                        check_endgame(game_settings, screen, w_u, h_u)


# passes logic to either easy, normal, or dard ai, to determine which square
# to fill
def ai_turn(game_settings, screen, w_u, h_u):
    xo = "O" if game_settings.x_or_o_ == "x" else "X"
...
def easy_ai(game_settings, screen, w_u, h_u, xo):
    ...
    ai_turn_end(game_settings, screen, w_u, h_u, xo, ai_turn)
...
# slightly more complicated than easy and normal, asks another module below
# to calculate what square to fill in
...
def ai_turn_end(game_settings, screen, w_u, h_u, xo, ai_turn):
    game_settings.ai_squares.add(str(ai_turn))
    gs.create_onscreen_text(game_settings, screen, w_u * (ai_turn[0] + 0.6),
                            h_u * (ai_turn[2] + 0.1), xo, 1)
    pygame.display.update()
    game_settings.players_turn = True
    check_endgame(game_settings, screen, w_u, h_u)


# checks whether one of the game ending conditions is fulfilled
# (player won, AI won, or draw)
...
# if so, it fills in the square that leads the player to victory, preventing
# them from winning # if not, it check whether the AI is a single step away
# from victory # if so, it fills in that square and wins # if both conditions
# are false, it simply chooses a random square this code can be expanded to
# cover two steps ahead, but that would make winning against mathematically
# impossible here, the goal was to make the ai harded than "easy" and
# "normal", but not impossible to win against
